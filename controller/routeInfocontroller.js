...
const getVillageMap = async (villageNames) => {
  const villageRecords = await villageadd.find({ village: { $in: villageNames } }).lean();
...
async function routeDetails(req, res) {
  ...
  try {
    ...
    if (decoded.email !== 'vinay') {
      return res.status(403).json({ error: "Access denied. You are not authorized to update bus details." });
    }
...
console.log(decoded,"decoded");
    if (decoded.email !== 'vinay') {
      return res.status(403).json({ error: "Access denied. You are not authorized to view bus details." });
    }
    ...
    if (date) {
      ...
      const busdetails = await Businfo.find({ date: { $gte: startOfDay, $lte: endOfDay } });
      return res.status(200).json({ data: busdetails });
    }
...
async function routeupdate(req, res) {
  ...
  try {
    ...
    const { date, fromtime, droptime, to, from, price, first, last, Busname, location, driver, cabinprice } = req.body;

    if (!date || !from || !to || !Busname || !price) {
      return res.status(400).json({ message: "All required fields must be provided." });
    }

    if (!Array.isArray(from) || !Array.isArray(to)) {
      return res.status(400).json({ message: "'from' and 'to' must be arrays of village data." });
    }
